...
import { BookOpen, Sun, Home, Compass, Clock, Menu } from "lucide-react";
...
const leftItems: { path: string; icon: typeof BookOpen; labelKey: TranslationKey }[] = [
  ...
  { path: "/dua", icon: Sun, labelKey: "nav.dua" },
];
...
const BottomNav = ({ onSettingsClick }: BottomNavProps) => {
  ...
  const isAdhkarActive = location.pathname === "/adhkar";
    ...
    >
      <div className="flex items-center justify-around py-1.5 relative">
        ...
        <button
          onClick={() => navigate("/")}
          className={`flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-xl transition-all duration-200 min-w-[3rem] ${
            isHomeActive ? "text-primary scale-105" : "text-muted-foreground hover:text-foreground"
          }`}
        >
          <div className={`p-1.5 rounded-xl transition-all ${isHomeActive ? "islamic-gradient text-primary-foreground shadow-md" : ""}`}>
            <Home className="w-5 h-5" strokeWidth={isHomeActive ? 2.2 : 1.8} />
          </div>
        ...
        >
          <div className={`w-14 h-14 rounded-full flex items-center justify-center shadow-lg transition-all ${
            isAdhkarActive
              ...
              : "islamic-gradient opacity-90 hover:opacity-100"
          }`}>
            <Sun className="w-6 h-6 text-primary-foreground" strokeWidth={2} />
          </div>
          <span className={`text-[10px] font-medium mt-0.5 ${isAdhkarActive ? "text-primary" : "text-muted-foreground"}`}>
            {t("nav.adhkar")}
          </span>
        </button>
